...
describe('SplitWise AI Assistant API (/api/ai/chat)', () => {
  ...
  it('successfully answers user query with structured response shape', async () => {
    ...
    expect(data.answer).toBe('You currently have no outstanding debts across your groups.');
  ...
  it('detects Hinglish queries and returns Hinglish language tag', async () => {
    ...
    expect(res.body.data.answer).toBe('Aapko Rahul ko koi paisa nahi dena hai.');
  });

  it('scopes data retrieval to user groups and respects financial integrity', async () => {
    const userA = await signupUser('usera@example.com', undefined, 'User Alpha');
    ...
    const callGeminiSpy = vi.spyOn(geminiClient, 'callGemini').mockImplementation(async (input) => {
    ...
    expect(data.sources.some((s: { type: string }) => s.type === 'member' || s.type === 'group')).toBe(true);
  });

  it('gracefully falls back to verified PostgreSQL data synthesis if Gemini call fails', async () => {
    const userA = await signupUser('fallbackuser@example.com', undefined, 'Fallback User');

    // Simulate Gemini API outage
    vi.spyOn(geminiClient, 'callGemini').mockRejectedValue(new Error('503 Service Unavailable'));
