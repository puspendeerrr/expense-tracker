...
export const QRImageCropperModal: React.FC<QRImageCropperModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in-0">
      <div className="bg-card rounded-2xl shadow-2xl border border-border w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border bg-muted/50">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-lg bg-emerald-500/15 flex items-center justify-center text-emerald-800 dark:text-emerald-300">
              <Crop className="h-4 w-4" />
            </div>
            <div>
              <h3 className="text-base font-bold text-foreground">Crop Payment QR</h3>
              <p className="text-xs text-muted-foreground">Center your QR code within the square frame</p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="h-8 w-8 rounded-lg text-muted-foreground hover:text-foreground hover:bg-accent flex items-center justify-center transition-colors"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-5 space-y-4 overflow-y-auto">
          {errorMessage && (
            <div className="p-3 text-xs bg-red-500/10 text-red-700 dark:text-red-400 border border-red-500/30 rounded-xl">
              {errorMessage}
            </div>
          )}
          ...
          <div className="flex flex-col items-center justify-center bg-slate-950 rounded-2xl p-3 relative overflow-hidden shadow-inner">
            ...
            />
            <p className="text-[11px] text-muted-foreground mt-2 flex items-center gap-1.5 select-none">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={zoom <= 1.0}
                className="h-8 w-8 rounded-lg border border-border text-muted-foreground hover:bg-accent flex items-center justify-center disabled:opacity-40"
              >
              ...
              <input
                ...
                className="flex-1 h-2 bg-accent rounded-lg appearance-none cursor-pointer accent-emerald-700"
              />
              <button
                type="button"
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className="px-2 py-2 rounded-xl border border-border text-xs font-semibold text-foreground/80 hover:bg-accent flex items-center justify-center gap-1.5 transition-colors"
            >
          ...
          {/* Tip */}
          <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[11px] text-emerald-900 dark:text-emerald-300 leading-relaxed">
            Ensure the complete QR code and UPI details remain inside the frame for reliable payment scanning.
          </div>
        </div>

        {/* Footer Actions */}
        <div className="px-5 py-3 border-t border-border bg-muted/50 flex items-center justify-end gap-2.5">
